...
export default function ExportData({ events }) {
  ...
  const calculateTotals = () => {
    ...
    const totalAcademic = events.filter(
      (event) => event.eventType === "Academic"
    ).length;
    const totalNonAcademic = events.filter(
    ...
    const totalMale = events.reduce(
      (sum, event) =>
        sum + (event.participants?.filter((p) => p.sex === "Male").length || 0),
      0
    );
    const totalFemale = events.reduce(
      (sum, event) =>
        sum +
        (event.participants?.filter((p) => p.sex === "Female").length || 0),
      0
    );
    return {
      totalEvents,
        ...
        .map(
          (event) =>
            `"${event.eventName}","${new Date(
              ...
              hour: "2-digit",
              minute: "2-digit",
            })}","${new Date(event.eventTimeTo).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })}","${event.eventVenue}","${event.eventType}","${
              event.eventCategory
            }","${event.numberOfHours}","${event.participants.length}","${
