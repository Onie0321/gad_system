...
import { AlertCircle, Download, ChevronUp, ChevronDown, Plus, Trash2, Edit2, RefreshCw, Save, Upload, ArrowUpDown, Filter } from "lucide-react"
...
export default function AdvancedFuturisticExcelTable() {
      ...
      reader.onload = (evt) => {
        try {
          ...
          if (parsedData.length > 0) {
            ...
            const nonEmptyRows = parsedData.slice(1).filter(row => row.some(cell => cell !== "" && cell !== null))
            setData(nonEmptyRows)
            calculateChartData(nonEmptyRows, parsedData[0])
            setError(null)
            const newImport = { id: Date.now(), name: fileName, date: new Date().toLocaleString() }
  ...
  const calculateChartData = useCallback((data, headers) => {
    ...
    headers.forEach((header, index) => {
      if (typeof data[0][index] === 'number') {
        ...
        newChartData[header] = {
          type: 'line',
          data: data.map((row, i) => ({ name: `Row ${i + 1}`, value: row[index] }))
        }
      } else {
  ...
  const scrollTo = (direction) => {
    ...
    if (scrollArea) {
      ...
      const targetScroll = direction === 'top' ? 0 : scrollHeight - clientHeight
      ...
      const animateScroll = (currentTime) => {
        ...
        if (elapsedTime < duration) {
          ...
          const newScrollPosition = currentScroll + (targetScroll - currentScroll) * easeProgress
  ...
  const sortedAndFilteredData = [...data].filter(row => {
    return Object.entries(filters).every(([key, value]) => {
      const index = headers.indexOf(key)
      return value === '' || row[index].toString().toLowerCase().includes(value.toLowerCase())
    })
  }).sort((a, b) => {
    if (sortConfig.key === null) return 0
    const index = headers.indexOf(sortConfig.key)
    if (a[index] < b[index]) return sortConfig.direction === 'ascending' ? -1 : 1
    if (a[index] > b[index]) return sortConfig.direction === 'ascending' ? 1 : -1
    return 0
  })
  ...
  const handleFileNameChange = (e) => {
    ...
    setRecentImports(prev => prev.map(import_ => 
      import_.id === selectedFile?.lastModified ? { ...import_, name: newFileName } : import_
    ))
  }
  ...
  return (
    <div className="container mx-auto p-4 space-y-4 bg-gray-100 rounded-lg shadow-lg">
      <h1 className="text-3xl font-bold mb-4 text-black text-center">Advanced Futuristic Excel Table</h1>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Are you sure you want to import this Excel file? This action will replace the current data.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <Input
              type="file"
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={handleFileUpload}>Import</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </div>
      ...
      {headers.length > 0 && (
        <div className="relative border-2 border-purple-500 rounded-lg p-4 bg-white">
          ...
          <ScrollArea className="h-[400px] rounded-md" ref={scrollAreaRef}>
            <Table>
              <TableHeader ref={headerRef} className="sticky top-0 bg-gray-100 z-10">
                <TableRow>
                  {headers.map((header, index) => (
                    <TableHead key={index} className={`font-bold text-black border-2 ${cellColors[index % cellColors.length]}`}>
                      <div className="flex items-center justify-between">
                        <span>{header}</span>
                        <div className="flex items-center">
                          <Button variant="ghost" size="sm" onClick={() => handleSort(header)}>
                            ...
                            <PopoverContent className="w-80">
                              <div className="grid gap-4">
                                <div className="space-y-2">
                                  <h4 className="font-medium leading-none">Filter {header}</h4>
                                ...
                                <div className="grid gap-2">
                                  <div className="grid grid-cols-3 items-center gap-4">
                                    <Label htmlFor={`filter-${header}`}>Value</Label>
                                    <Input
                                      id={`filter-${header}`}
                  ...
                  <TableHead className="font-bold text-black border-2 border-gray-400">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {sortedAndFilteredData.map((row, rowIndex) => (
                  <TableRow key={rowIndex} className="hover:bg-gray-50">
                    {headers.map((header, cellIndex) => (
                      <TableCell key={cellIndex} className={`text-black border-2 ${cellColors[cellIndex % cellColors.length]}`}>
                        {editingCell?.row === rowIndex && editingCell?.cell === cellIndex ? (
                          <Input
                            value={row[cellIndex] !== undefined ? row[cellIndex] : ''}
                          ...
                          <span onClick={() => setEditingCell({ row: rowIndex, cell: cellIndex })} className="cursor-pointer block w-full h-full">
                            {row[cellIndex]}
                          </span>
                        )}
                      </TableCell>
                      ...
                      <div className="flex space-x-2">
                          ...
                          <AlertDialogContent>
                            ...
                            <div className="grid gap-4 py-4">
                              {headers.map((header, index) => (
                                <div key={index} className="grid grid-cols-4 items-center gap-4">
                                  <Label htmlFor={`row-${rowIndex}-${header}`} className="text-right">
                                    {header}
                                  </Label>
                                  <Input
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => {
                                const updatedRow = headers.map((_, index) => 
                                  document.getElementById(`row-${rowIndex}-${headers[index]}`).value
                                )
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              <AlertDialogCancel>Cancel</AlertDialogCancel>
                              <AlertDialogAction onClick={() => handleDeleteRow(rowIndex)}>Delete</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </div>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Are you sure you want to export this data to an Excel file?
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
          ...
          {!isScrolling && (
            <div className="absolute right-2 top-2 flex flex-col space-y-2">
              <Button onClick={() => scrollTo('top')} size="sm" variant="outline" className="bg-blue-500 hover:bg-blue-600 text-white">
                <ChevronUp className="h-4 w-4" />
              </Button>
              <Button onClick={() => scrollTo('bottom')} size="sm" variant="outline" className="bg-blue-500 hover:bg-blue-600 text-white">
      ...
      {headers.length > 0 && (
        <div className="mt-4 space-y-4">
          <div className="flex space-x-2 justify-center">
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Are you sure you want to add a new row to the top of the table?
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancel</AlertDialogCancel>
                  <AlertDialogAction onClick={handleAddRow}>Add</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
            <AlertDialog>
            ...
            <Button onClick={handleRefresh} className="bg-yellow-500 hover:bg-yellow-600 text-white">
          ...
          <div className="border-2 border-purple-500 rounded-lg p-4 bg-white">
            <h2 className="text-xl font-bold mb-2 text-black">Data Visualization</h2>
      ...
      {recentImports.length > 0 && (
        <div className="mt-4 border-2 border-purple-500 rounded-lg p-4 bg-white">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="font-bold text-black">File Name</TableHead>
                <TableHead className="font-bold text-black">Import Date</TableHead>
                  ...
                  <TableCell>
                    <div className="flex space-x-2">
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>Update Import Name</AlertDialogTitle>
                            <AlertDialogDescription>
                          ...
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction onClick={() => {
                              const newName = document.getElementById(`import-${import_.id}`).value
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Are you sure you want to delete this import from the recent list? This action cannot be undone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                            <AlertDialogAction onClick={() => handleDeleteRecentImport(import_.id)}>Delete</AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                    </div>
